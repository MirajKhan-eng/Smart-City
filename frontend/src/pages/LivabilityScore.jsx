...
const localityMap = {
  A: ["Colaba", "Nariman Point"],
  B: ["Dongri", "Sandhurst Road"],
  C: ["Marine Lines", "Kalbadevi"],
  D: ["Malabar Hill", "Grant Road"],
  E: ["Byculla", "Mazgaon"],
  "F/N": ["Matunga", "Sion"],
  "F/S": ["Parel", "Sewri"],
  "G/N": ["Dharavi", "Dadar"],
  "G/S": ["Worli", "Lower Parel"],
  "H/E": ["Santacruz East", "Bandra East"],
  "H/W": ["Bandra West", "Khar"],
  "K/E": ["Andheri East", "Jogeshwari"],
  "K/W": ["Andheri West", "Versova"],
  L: ["Kurla", "Saki Naka"],
  ...
  S: ["Bhandup", "Powai"],
  T: ["Mulund"],
  Belapur: ["CBD Belapur", "Sector 15"],
  Nerul: ["Seawoods", "Sector 40"],
  Vashi: ["Sagar Vihar", "APMC"],
  Sanpada: ["Palm Beach Road"],
  Koparkhairane: ["Sector 14"],
  Ghansoli: ["Sector 3"],
  Airoli: ["Mindspace"],
  Digha: ["Sathe Nagar"],
...
const StatRow = ({ label, val, icon, color, isComparison }) => (
  <div className="space-y-3">
    <div className="flex justify-between items-end px-1">
      ...
      <span
        className={`font-black text-white ${isComparison ? "text-sm" : "text-xl"}`}
      >
        {val}%
      </span>
    </div>
  ...
  <div className="flex flex-col h-full overflow-hidden">
    <div className="flex justify-between items-start mb-6 shrink-0">
      ...
      <div className="text-right">
        <h2
          className={`${isComparison ? "text-2xl" : "text-4xl"} font-black italic uppercase text-blue-500 leading-none`}
        >
          {area.displayLabel}
        </h2>
        <h3
          className={`${isComparison ? "text-sm" : "text-xl"} font-black text-white uppercase mt-2`}
        >
          {area.locality}
        </h3>
      </div>
    ...
    <div className="flex-1 flex flex-col justify-center py-4 space-y-8 min-h-0 overflow-y-auto custom-scrollbar">
      <div
        className={`relative ${isComparison ? "w-48 h-48" : "w-64 h-64"} mx-auto flex items-center justify-center shrink-0`}
      >
        <svg className="absolute w-full h-full -rotate-90">
          <circle
            cx={isComparison ? "96" : "128"}
            cy={isComparison ? "96" : "128"}
            r={isComparison ? "82" : "110"}
            stroke="#1e293b"
            strokeWidth={isComparison ? "14" : "20"}
            fill="none"
          />
          <circle
            cx={isComparison ? "96" : "128"}
            cy={isComparison ? "96" : "128"}
            r={isComparison ? "82" : "110"}
            stroke={getColor(area.overall_score)}
            strokeWidth={isComparison ? "14" : "20"}
            fill="none"
            strokeDasharray={isComparison ? "515" : "691"}
            strokeDashoffset={
              isComparison
                ? 515 - (515 * area.overall_score) / 100
                : 691 - (691 * area.overall_score) / 100
            }
        ...
        <div className="text-center">
          <span
            className={`${isComparison ? "text-6xl" : "text-8xl"} font-black italic text-white leading-none tracking-tighter`}
          >
            {area.overall_score}
          </span>
      ...
      <div className="space-y-6">
        ...
        <StatRow
          label="Environment"
          val={area.environment_score}
          icon={<Trees className="w-4 h-4 text-cyan-400" />}
          color="bg-cyan-500"
          isComparison={isComparison}
        />
      </div>
    ...
    {!isComparison && (
      <button
        onClick={onInsights}
        className="mt-8 w-full py-5 bg-blue-600 text-white rounded-3xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-4 hover:bg-blue-700 transition-all shrink-0"
      >
        Analytical Matrix <ArrowRight size={16} />
      </button>
    )}
  </div>
);
...
const InsightCard = ({ title, score, icon }) => (
  <div className="bg-slate-900/50 p-10 rounded-[3rem] border border-white/5 flex flex-col justify-between hover:bg-blue-600/5 transition-all">
    <div>
      ...
      <h4 className="text-white font-black uppercase text-lg mb-2 tracking-tighter">
        {title}
      </h4>
    ...
    <div className="flex items-baseline gap-4 mt-8">
      <span className="text-5xl font-black italic text-white">{score}</span>
...
const LivabilityScore = () => {
  ...
  useEffect(() => {
    fetch("https://smart-city-1-42tj.onrender.com/api/livability_all")
      ...
      .then((data) => {
        const scoreMap = {};
        data.forEach((item) => {
          scoreMap[item.area_name.toUpperCase().trim()] = item.overall_score;
        });
  ...
  const handleAreaClick = async (feature) => {
    ...
    const localities =
      localityMap[displayId] ||
      localityMap[id] ||
      (p.Name ? [`Sector ${p.Name}`] : ["Main Locality"]);
    ...
    try {
      ...
      const factors = [
        {
          title: "Public Safety",
          val: data.safety_score,
          icon: <ShieldCheck />,
        },
        {
          title: "Health Facilities",
          val: Math.min(100, data.safety_score + 8),
          icon: <Building2 />,
        },
        {
          title: "Academic Hubs",
          val: Math.min(100, data.environment_score - 4),
          icon: <GraduationCap />,
        },
        { title: "Transit Network", val: data.mobility_score, icon: <Zap /> },
        {
          title: "Green Coverage",
          val: data.environment_score,
          icon: <Trees />,
        },
      ];
      const areaData = {
      ...
      if (comparisonMode) {
        if (!selectedArea) setSelectedArea(areaData);
        else setCompareArea(areaData);
      } else {
  ...
  return (
    <div className="flex h-screen w-full bg-[#030712] overflow-hidden font-sans">
        ...
        >
          ...
          <div className="absolute top-10 left-10 z-[1000] flex flex-col gap-6">
            <div className="bg-slate-900/95 backdrop-blur-3xl p-6 rounded-[2rem] border border-white/10 shadow-2xl">
              <h1 className="text-2xl font-black italic text-white flex items-center gap-3">
                <Activity className="text-blue-500 w-6 h-6" /> LIVE INDEX
              </h1>
            ...
            <button
              ...
              className={`px-8 py-4 rounded-[2rem] font-black text-[10px] uppercase tracking-widest flex items-center gap-4 transition-all shadow-xl ${comparisonMode ? "bg-orange-600 text-white shadow-orange-500/20" : "bg-slate-900 text-slate-300 border border-white/10"}`}
            >
              <ArrowLeftRight size={16} />{" "}
              {comparisonMode ? "Exit Mode" : "Compare Areas"}
            </button>
            <LiveLocationControl
              combinedData={combinedData}
              onAreaFound={handleAreaClick}
            />
          </div>
          <GeoJSON
            key={`${Object.keys(allScores).length}-${comparisonMode}-${!!selectedArea}`}
            data={combinedData}
            style={(f) => ({
              fillColor: getColor(allScores[getIdentifier(f)]),
              weight: 1.5,
              color: "white",
              fillOpacity: 0.5,
            })}
      ...
      {selectedArea && (
        <div
          className={`fixed right-0 top-0 h-full bg-slate-950/95 backdrop-blur-3xl border-l border-white/10 transition-all duration-700 z-[2000] ${comparisonMode ? "w-[650px]" : showInsights ? "w-full" : "w-[450px]"} flex shadow-2xl overflow-hidden`}
        >
          <div
            className={`${comparisonMode ? "w-1/2" : "w-full"} p-8 flex flex-col border-r border-white/5 overflow-hidden`}
          >
            <AreaSummary
              area={selectedArea}
              onDismiss={() => {
          ...
          {comparisonMode && (
            <div className="w-1/2 p-8 flex flex-col bg-slate-900/10">
              {compareArea ? (
                <AreaSummary
                  area={compareArea}
                  onDismiss={() => setCompareArea(null)}
                  onInsights={() => {}}
                  isComparison={true}
                />
              ) : (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-6 space-y-4">
                  <div className="w-16 h-16 bg-blue-600/10 rounded-full flex items-center justify-center text-blue-500 animate-pulse">
                    <GitCompare size={32} />
                  </div>
          ...
          {showInsights && !comparisonMode && (
            <div className="absolute inset-0 bg-slate-950 p-20 overflow-y-auto animate-in fade-in duration-500 z-50">
              <div className="flex justify-between items-center mb-16">
                <div>
                  <h3 className="text-6xl font-black italic text-white uppercase leading-none tracking-tighter">
                    Analytical Matrix
                  </h3>
                  <p className="text-blue-500 text-2xl font-black mt-2 uppercase">
                    {selectedArea.displayLabel} / {selectedArea.locality}
                  </p>
                </div>
                <button
                  onClick={() => setShowInsights(false)}
                  className="px-8 py-4 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase text-slate-400 hover:text-white transition-all"
                >
                  Close
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {selectedArea.factors.map((f, i) => (
                  <InsightCard
                    key={i}
                    title={f.title}
                    score={f.val}
