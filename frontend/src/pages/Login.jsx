...
const Login = () => {
  ...
  const [formData, setFormData] = useState({ email: '', password: '', deptId: '' });
    ...
    <div className="min-h-screen bg-[#FBFBFE] flex items-center justify-center p-6">
      <div className="max-w-xl w-full">
        ...
        <div className="flex bg-white p-2 rounded-[2rem] shadow-sm mb-10 border border-slate-100">
            ...
            <button 
                onClick={() => { setIsOfficial(true); setError(''); }}
        ...
        <div className="bg-white rounded-[3.5rem] p-12 shadow-2xl shadow-slate-200 border border-slate-100 relative overflow-hidden">
            <div className="text-center mb-12">
                <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-xl ${isOfficial ? 'bg-blue-50 text-blue-600' : 'bg-slate-50 text-slate-900'}`}>
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">
                    {isOfficial ? 'Secured SmartCity Node' : 'Personal Dashboard Login'}
                </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 ml-2 tracking-widest">Email Identity</label>
                    <div className="relative">
                        <input
                            id="email"
                            name="email"
                            type="email"
                            placeholder={isOfficial ? "officer@mumbai.gov.in" : "citizen@example.com"}
                ...
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 ml-2 tracking-widest">Secured Password</label>
                    <input
                        id="password"
                        name="password"
                        type="password"
            ...
            {isOfficial && (
                <div className="mt-8 p-4 bg-slate-50 rounded-2xl flex items-center gap-3 border border-slate-100">
                    ...
                    <p className="text-[8px] font-bold text-slate-400 uppercase leading-relaxed tracking-widest">Unauthorized access to official nodes is strictly monitored and logged.</p>
                </div>
            )}
        </div>
      </div>
