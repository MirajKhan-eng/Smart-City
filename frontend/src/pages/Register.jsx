...
const Register = () => {
  ...
  return (
    <div className={`max-w-md mx-auto mt-10 p-8 bg-white rounded-2xl shadow-xl border-t-8 ${formData.role === 'admin' ? 'border-red-600' : 'border-blue-600'}`}>
      <h2 className="text-3xl font-bold mb-2 text-center text-gray-800">
        {formData.role === 'admin' ? 'Official Registration' : 'Citizen Sign Up'}
      </h2>
      <p className="text-center text-gray-500 mb-6 italic">
        {formData.role === 'admin' ? 'Authorized City Personnel Only' : 'Join the Smart City Network'}
      </p>

      {error && <div className="bg-red-50 text-red-600 p-3 mb-4 rounded-lg text-sm border border-red-200 font-medium text-center">{error}</div>}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {formData.role === 'admin' && (
          <div className="bg-red-50 p-4 rounded-xl border border-red-100">
            <label className="block text-xs font-bold text-red-700 mb-1 uppercase tracking-wider">Departmental Admin ID</label>
            <input
              type="text"
              ...
              required
            />
            <p className="text-[10px] text-red-500 mt-2 font-medium">This ID is required for administrative status.</p>
          </div>
        )}
        ...
        >
          {formData.role === 'admin' ? 'Authorize & Register' : 'Register as Citizen'}
        </button>
      </form>
    </div>
  );
