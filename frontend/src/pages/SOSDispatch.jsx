...
import { 
    Shield, Clock, MapPin, AlertCircle, Phone, Radio, Navigation, 
    ShieldAlert, Flame, Stethoscope, CheckCircle, Activity, Zap, Info,
    X, ChevronRight, Loader2, ShieldCheck
} from 'lucide-react';
...
const SOSDispatch = () => {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      const sosAlerts = res.data.filter(r => r.type === 'Emergency' || r.title === 'SOS ALERT');
      setAlerts(sosAlerts);
  ...
  const handleDispatch = async (type) => {
    ...
    try {
        await axios.put(`http://localhost:5000/api/admin/dispatch/${selectedAlert.id}`, {
            type: type,
            eta: eta,
    ...
    } catch (err) { alert("Dispatch Failed"); setDispatchStatus(null); }
  };

  const resolveAlert = async (id) => {
    try {
        await axios.put(`http://localhost:5000/api/admin/reports/${id}`, { status: 'Resolved', tracking_step: 6 });
        fetchAlerts();
  ...
  const getCoords = (loc) => {
    if (!loc) return [19.0760, 72.8777];
    const parts = loc.split(',').map(Number);
    return parts.length === 2 ? parts : [19.0760, 72.8777];
  };
        ...
        <div className="flex items-center gap-6">
            ...
            <div>
                <h1 className="text-2xl font-black text-white italic uppercase tracking-tighter flex items-center gap-3">
                    Mission Control <span className="text-red-500 underline underline-offset-4">SOS Dispatch</span>
                </h1>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mt-1 flex items-center gap-2">
                    <Activity size={12} className="text-emerald-500" /> Sector: Mumbai Metropolitan Area
                </p>
            </div>
        </div>
        ...
        <aside className="w-[450px] bg-slate-900/30 border-r border-white/5 overflow-y-auto custom-scrollbar">
            <div className="p-8">
                <div className="flex items-center gap-3">
                    ...
                    >
                        <Radio size={12} className={loading ? 'animate-pulse' : ''} /> Force Sync
                    </button>
                    <span className="text-[9px] font-black px-3 py-1 bg-red-500/10 text-red-500 rounded-full">{alerts.length} Distressed Nodes</span>
                </div>
                ...
                <div className="space-y-4">
                    {alerts.map(alert => (
                        ...
                        >
                            <div className="flex justify-between items-start mb-4">
                                <div className={`p-3 rounded-xl ${selectedAlert?.id === alert.id ? 'bg-white/20' : 'bg-red-500/10 text-red-500'}`}>
                                    <AlertCircle size={20} className={alert.status === 'Pending' ? 'animate-pulse' : ''} />
                                </div>
                                <div className="text-right">
                                    <span className={`block text-[8px] font-black uppercase tracking-widest ${selectedAlert?.id === alert.id ? 'text-white' : 'text-slate-500'}`}>ID: #{alert.id}</span>
                                    <span className={`text-[8px] font-black uppercase px-2 py-1 rounded-md ${selectedAlert?.id === alert.id ? 'bg-white/20 text-white' : 'bg-red-500/20 text-red-500'}`}>CRITICAL</span>
                                </div>
                            </div>
                            <h4 className={`text-lg font-black italic tracking-tight uppercase leading-none mb-2 ${selectedAlert?.id === alert.id ? 'text-white' : 'text-white'}`}>{alert.title}</h4>
                            <div className="flex items-center gap-3">
                                <div className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-tighter ${selectedAlert?.id === alert.id ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-400'}`}>
                                    {alert.type || 'General SOS'}
                                </div>
                                <p className={`text-[10px] font-bold uppercase truncate ${selectedAlert?.id === alert.id ? 'text-white/70' : 'text-slate-500'}`}><MapPin size={10} className="inline mr-1" /> {alert.location}</p>
                            </div>
                    ...
                    {alerts.length === 0 && (
                        <div className="py-20 text-center space-y-4">
                            ...
                            <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Awaiting new distress nodes...</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            {selectedAlert ? (
                <>
                    ...
                    <div className="flex-1 bg-slate-800 relative group">
                        ...
                        >
                            ...
                            <Circle center={getCoords(selectedAlert.location)} radius={300} pathOptions={{ color: 'red', fillColor: 'red', fillOpacity: 0.1 }} />
                            <Marker position={getCoords(selectedAlert.location)}>
                                <Popup><span className="font-black text-xs uppercase">Distress Node: {selectedAlert.id}</span></Popup>
                            </Marker>
                        </MapContainer>
                        ...
                        <div className="absolute top-10 left-10 z-[1000] bg-slate-900/80 backdrop-blur-xl p-8 rounded-[3rem] border border-white/10 w-96 shadow-2xl">
                            ...
                            <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter mb-4">{selectedAlert.reporter_name || 'Anonymous Citizen'}</h3>
                            <div className="space-y-4 mb-8">
                                <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                                    <p className="text-[8px] font-black text-slate-500 uppercase mb-1">Coordinated Address</p>
                                    <p className="text-xs font-bold text-white leading-relaxed">{selectedAlert.location}</p>
                                </div>
                                <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                                    <p className="text-[8px] font-black text-slate-500 uppercase mb-1">Status Report</p>
                                    <p className="text-xs font-bold text-slate-300 italic">"Panic button triggered via mobile app. Citizen requires immediate tactical support."</p>
                                </div>
                            </div>
                            <button onClick={() => resolveAlert(selectedAlert.id)} className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-xl">
                ...
                <div className="flex-1 flex flex-col items-center justify-center space-y-8">
                    ...
                    <div className="text-center">
                        <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">Awaiting Signal</h2>
                        <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] mt-3">Select a node from the live feed to begin tactical dispatch</p>
                    </div>
                </div>
            )}
        </main>
