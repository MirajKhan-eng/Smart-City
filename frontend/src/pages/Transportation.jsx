...
const AddressAutocomplete = ({
  ...
  return (
    <div className="relative group w-full">
      <Icon
        className={`absolute left-4 top-1/2 -translate-y-1/2 ${colorClass} transition-colors z-10`}
        size={16}
      />
      <input
      ...
      {loading && (
        <Loader2
          className="absolute right-4 top-1/2 -translate-y-1/2 animate-spin text-blue-500 z-10"
          size={14}
        />
      )}
        ...
        <div className="space-y-4 mb-10">
          ...
          >
            {loading ? (
              ...
              <>
                Calculate Best Route <ArrowRight size={16} />
              </>
            )}
          </button>
        </div>
            ...
            {data.options.map((opt, i) => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div
                    className={`p-3 rounded-2xl ${selectedMode?.mode === opt.mode ? "bg-blue-600 text-white" : "bg-white/5 text-slate-400 group-hover:text-white"}`}
                  >
                    {opt.mode === "Train" ? (
                      <Train size={18} />
                    ) : opt.mode === "Bus" ? (
        ...
        {data && selectedMode && (
          <div className="absolute top-10 right-10 flex flex-col gap-4 z-[1000] w-80 animate-in fade-in slide-in-from-right duration-700">
            <div className="bg-white/95 backdrop-blur-xl p-6 rounded-[2.5rem] shadow-2xl border border-white/20">
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                <div className="flex-1">
                  ...
                  <div
                    className={`text-xl font-black italic ${selectedMode.crowd === "Low" ? "text-emerald-500" : "text-rose-500"}`}
                  >
                    {selectedMode.crowd}
                  </div>
                </div>
            ...
            {data.alerts.map((alert, i) => (
              <div
                key={i}
                className="bg-rose-600 p-5 rounded-[2rem] shadow-2xl flex gap-4 items-start border border-rose-500"
              >
                <AlertCircle size={20} className="text-white shrink-0" />
                <div>
                  <p className="text-[9px] font-black text-rose-100 uppercase tracking-widest">
                    {alert.title}
                  </p>
